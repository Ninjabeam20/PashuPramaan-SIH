...
type TabId = "overview" | "analytics" | "anomalies" | "health" | "forecast" | "workspace"

// ─── Geographic Data ─────────────────────────────────────────────────────────

const STATE_GEO = [
  { id:"JK",  name:"J&K & Ladakh",      d:"M60,2 L146,2 L150,22 L124,56 L80,56 L56,28 Z",                cx:103, cy:28  },
  { id:"HP",  name:"Himachal Pradesh",   d:"M80,56 L124,56 L132,80 L96,92 L80,80 Z",                      cx:108, cy:72  },
  { id:"PB",  name:"Punjab",             d:"M56,56 L80,56 L80,106 L56,106 Z",                             cx:68,  cy:80  },
  { id:"UT",  name:"Uttarakhand",        d:"M124,56 L156,68 L156,94 L132,80 Z",                           cx:140, cy:76  },
  { id:"HR",  name:"Haryana",            d:"M68,80 L108,80 L108,116 L68,116 Z",                           cx:88,  cy:98  },
  { id:"DL",  name:"Delhi",              d:"M108,94 L118,94 L118,108 L108,108 Z",                         cx:113, cy:101 },
  { id:"RJ",  name:"Rajasthan",          d:"M8,80 L68,80 L68,116 L108,116 L108,164 L8,164 Z",            cx:52,  cy:126 },
  { id:"UP",  name:"Uttar Pradesh",      d:"M108,80 L192,80 L192,164 L108,164 Z",                        cx:150, cy:122 },
  { id:"BR",  name:"Bihar",              d:"M192,104 L240,104 L240,152 L192,152 Z",                       cx:216, cy:128 },
  { id:"JH",  name:"Jharkhand",          d:"M192,152 L228,152 L228,198 L192,198 Z",                      cx:210, cy:174 },
  { id:"WB",  name:"West Bengal",        d:"M228,104 L256,104 L256,208 L216,208 L216,152 L228,152 Z",    cx:240, cy:156 },
  { id:"NE",  name:"North-East",         d:"M256,92 L350,92 L350,190 L256,190 Z",                        cx:300, cy:141 },
  { id:"OD",  name:"Odisha",             d:"M168,194 L228,194 L228,256 L168,256 Z",                      cx:198, cy:225 },
  { id:"MP",  name:"Madhya Pradesh",     d:"M68,138 L192,138 L192,196 L68,196 Z",                       cx:130, cy:168 },
  { id:"CG",  name:"Chhattisgarh",       d:"M168,164 L204,164 L204,256 L168,256 Z",                      cx:186, cy:210 },
  { id:"GJ",  name:"Gujarat",            d:"M2,164 L68,164 L68,212 L34,224 L2,212 Z",                   cx:36,  cy:190 },
  { id:"MH",  name:"Maharashtra",        d:"M34,210 L168,210 L168,274 L52,274 L34,250 Z",                cx:100, cy:243 },
  { id:"TG",  name:"Telangana",          d:"M108,214 L168,214 L168,270 L108,270 Z",                      cx:138, cy:242 },
  { id:"AP",  name:"Andhra Pradesh",     d:"M108,254 L228,254 L218,306 L96,306 Z",                       cx:162, cy:278 },
  { id:"KA",  name:"Karnataka",          d:"M52,248 L108,248 L108,308 L76,320 L52,298 Z",                cx:80,  cy:280 },
  { id:"GA",  name:"Goa",               d:"M52,274 L70,274 L70,288 L52,288 Z",                          cx:61,  cy:281 },
  { id:"KL",  name:"Kerala",             d:"M70,304 L96,304 L96,352 L70,352 Z",                          cx:83,  cy:327 },
  { id:"TN",  name:"Tamil Nadu",         d:"M96,292 L178,292 L170,352 L96,352 Z",                        cx:134, cy:322 },
]
...
const amuById = Object.fromEntries(REGION_DATA.map(r => [r.id, r]))

interface DistrictRow {
  district: string; amu: number; change: number; anomalies: number; unexplained: number;
}

const DISTRICT_DATA: Record<string, DistrictRow[]> = {
  GJ: [
...
const MONTHLY_AMU     = [112, 108, 115, 110, 114, 118, 156, 182]
...
function amuFill(amu: number): string {
  if (amu < 25000) return "#D4ECD8"
  if (amu < 45000) return "#B8DFC0"
  if (amu < 65000) return "#FEF3C7"
  if (amu < 80000) return "#FDDCB0"
  return "#F9C4A0"
}
function amuStroke(amu: number): string {
  if (amu < 25000) return "#8ECA98"
  if (amu < 45000) return "#6CB87A"
  if (amu < 65000) return "#E8B400"
  if (amu < 80000) return "#D47820"
  return "#C05010"
}

function changePill(change: number) {
...
interface AttentionCardProps { title:string; sub:string; level:"HIGH"|"MEDIUM"|"LOW"|"INFO"; link:string; icon:string }
function AttentionCard({ title, sub, level, link, icon }: AttentionCardProps) {
  ...
  return (
    <div style={{ background:"#fff", borderRadius:8, border:"1px solid #E8E4DC", borderLeft:`4px solid ${a.border}`, padding:"14px 16px", display:"flex", flexDirection:"column", gap:8 }}>
      ...
      <button style={{ fontSize:12, fontWeight:500, color:"#2D6A4F", background:"none", border:"none", cursor:"pointer", fontFamily:"inherit", textAlign:"left", padding:0 }}>{link} →</button>
    </div>
  )
}
...
function IndiaMap({ highlightId, onHover, onClick }: { highlightId:string|null; onHover:(id:string|null)=>void; onClick?:(id:string)=>void }) {
  ...
  const [tip, setTip] = useState<{x:number;y:number;id:string;name:string}|null>(null)
  const row = tip ? amuById[tip.id] : null

  return (
    <div style={{ position:"relative", width:"100%" }}>
      <svg ref={svgRef} viewBox="0 0 370 380" style={{ width:"100%", height:"auto", display:"block" }}>
        <rect width="370" height="380" fill="#EAF3FB" rx="4"/>
        {STATE_GEO.map(geo => {
          const d = amuById[geo.id]; const amu = d?.amu ?? 0; const hov = highlightId === geo.id
          return (
            <path key={geo.id} d={geo.d}
              fill={hov ? "#2D6A4F" : amuFill(amu)} stroke={hov ? "#1A4030" : amuStroke(amu)}
              strokeWidth={hov ? 1.4 : 0.8} strokeLinejoin="round"
              style={{ cursor: onClick ? "pointer" : "default", transition:"fill 0.12s" }}
              onMouseMove={e => { const r = svgRef.current?.getBoundingClientRect(); if(!r) return; setTip({x:e.clientX-r.left,y:e.clientY-r.top,id:geo.id,name:geo.name}); onHover(geo.id) }}
              onMouseLeave={() => { setTip(null); onHover(null) }}
              onClick={() => onClick?.(geo.id)}/>
          )
        })}
        {STATE_GEO.filter(g => ["RJ","UP","MP","MH","GJ","KA","AP","TN","WB","NE","BR","OD","CG"].includes(g.id)).map(geo => (
          <text key={`l-${geo.id}`} x={geo.cx} y={geo.cy+3} textAnchor="middle"
            style={{ fontSize:7, fontFamily:"Inter, system-ui", fontWeight:600, fill:highlightId===geo.id?"#fff":"#374151", pointerEvents:"none" }}>
            {geo.id}
          </text>
        ))}
      </svg>
      {tip && row && (
        <div style={{ position:"absolute", left:tip.x+12, top:tip.y-8, background:"#1A2E24", borderRadius:8, padding:"10px 14px", minWidth:160, boxShadow:"0 4px 16px rgba(0,0,0,0.18)", pointerEvents:"none", zIndex:20 }}>
          <p style={{ color:"#fff", fontWeight:600, fontSize:12, marginBottom:6 }}>{tip.name}</p>
      ...
      <div style={{ position:"absolute", bottom:8, right:8, background:"rgba(255,255,255,0.92)", borderRadius:6, padding:"7px 10px", border:"1px solid #E8E4DC" }}>
        <p style={{ fontSize:9, fontWeight:600, color:"#9CA3AF", letterSpacing:"0.06em", textTransform:"uppercase", marginBottom:4 }}>AMU Volume</p>
        {[{c:"#D4ECD8",l:"< 25k"},{c:"#B8DFC0",l:"25–45k"},{c:"#FEF3C7",l:"45–65k"},{c:"#FDDCB0",l:"65–80k"},{c:"#F9C4A0",l:"> 80k"}].map(({c,l}) => (
          <div key={l} style={{ display:"flex", alignItems:"center", gap:5, marginBottom:2 }}>
            <div style={{ width:9, height:9, background:c, borderRadius:2, border:"1px solid rgba(0,0,0,0.1)" }}/>
...
function DistrictGrid({ stateId, highlightId, onHover }: { stateId:string; highlightId:string|null; onHover:(id:string|null)=>void }) {
...
function SparklineChart({ data, baseline }: { data: number[]; baseline: number }) {
    ...
    <div style={{ maxWidth:1200, margin:"0 auto", padding:"32px 24px 48px" }}>
      ...
      <div style={{ marginBottom:24 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"baseline", marginBottom:12 }}>
          ...
          <button style={{ fontSize:13, fontWeight:500, color:"#2D6A4F", background:"none", border:"none", cursor:"pointer", fontFamily:"inherit" }}>View all anomalies →</button>
        </div>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(280px, 1fr))", gap:12 }}>
          <AttentionCard icon="⚠️" level="HIGH"   title="5 unexplained AMU anomalies require investigation" sub="No correlated health event · flagged by system" link="Review anomalies"/>
          <AttentionCard icon="📈" level="MEDIUM" title="Maharashtra demand predicted to increase"          sub="Model confidence: 87% · Next 30 days" link="View forecast"/>
          <AttentionCard icon="💊" level="MEDIUM" title="2 regions showing elevated CIA usage"             sub="Punjab 34% · Haryana 32% · Above threshold"   link="View CIA report"/>
          <AttentionCard icon="🔗" level="INFO"   title="12 anomalies linked to recorded health events"   sub="Associated with IBD, mastitis outbreaks"       link="View Health × AMU"/>
        </div>
      </div>
...
const INDIA_STATES_LIST = ["All States", ...REGION_DATA.map(r => r.state)]
...
function AnalyticsTab() {
  ...
  const [metricFilter, setMetricFilter] = useState("AMU")
  const [hoverId, setHoverId] = useState<string|null>(null)
    ...
    <div style={{ maxWidth:1300, margin:"0 auto", padding:"28px 24px 48px" }}>
      ...
      <Card style={{ padding:"14px 18px", marginBottom:18, display:"flex", gap:20, flexWrap:"wrap", alignItems:"flex-end" }}>
        <SelectField label="State" value={stateFilter} options={INDIA_STATES_LIST}
          onChange={v => { setStateFilter(v); setDistrictFilter("All Districts"); setDrillDistrict(null); setDrillStateId(v==="All States"?null:REGION_DATA.find(r=>r.state===v)?.id??null) }}/>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 272px", gap:16 }}>
        ...
        <Card style={{ overflow:"hidden" }}>
          ...
          {!drillStateId
            ...
            : <DistrictGrid stateId={drillStateId} highlightId={hoverId} onHover={setHoverId}/>
          }
        </Card>
...
function TreatmentRecordsModal({ anomaly, onClose }: { anomaly: AnomalyRow; onClose: () => void }) {
...
function AnomalyDetail({ anomaly, onBack, onSaveInsight }: { anomaly: AnomalyRow; onBack: () => void; onSaveInsight?: (ins: SavedInsight) => void }) {
...
function AnomaliesTab({ onSaveInsight, initialSelected }: { onSaveInsight?: (ins: SavedInsight) => void; initialSelected?: string|null }) {
...
const DEMAND_LEVEL: Record<string, { demand:"High"|"Medium"|"Low"; change:number; currentAmu:"High"|"Medium"|"Low"; signal:"High need"|"Monitor"|"Stable" }> = {
...
function demandFill(id: string): string {
  const d = DEMAND_LEVEL[id]?.demand
  if (d === "High")   return "#F9C4A0"
  if (d === "Medium") return "#FEF3C7"
  return "#D4ECD8"
}
function demandStroke(id: string): string {
  const d = DEMAND_LEVEL[id]?.demand
  if (d === "High")   return "#C05010"
  if (d === "Medium") return "#E8B400"
  return "#8ECA98"
}

const REGIONAL_PLANNING = [
...
// ─── Demand map (reuses STATE_GEO with demand coloring) ───────────────────────

function DemandMap({ highlightId, onHover }: { highlightId:string|null; onHover:(id:string|null)=>void }) {
  const svgRef = useRef<SVGSVGElement>(null)
  const [tip, setTip] = useState<{x:number;y:number;id:string;name:string}|null>(null)
  const d = tip ? DEMAND_LEVEL[tip.id] : null
  ...
  return (
    <div style={{ position:"relative", width:"100%" }}>
      <svg ref={svgRef} viewBox="0 0 370 380" style={{ width:"100%", height:"auto", display:"block" }}>
        <rect width="370" height="380" fill="#EAF3FB" rx="4"/>
        {STATE_GEO.map(geo => {
          const hov = highlightId === geo.id
          return (
            <path key={geo.id} d={geo.d}
              fill={hov ? "#2D6A4F" : demandFill(geo.id)}
              stroke={hov ? "#1A4030" : demandStroke(geo.id)}
              strokeWidth={hov ? 1.4 : 0.8} strokeLinejoin="round"
        ...
        {STATE_GEO.filter(g => ["RJ","UP","MP","MH","GJ","KA","AP","TN","WB","NE","BR","OD","CG"].includes(g.id)).map(geo => (
          <text key={`l-${geo.id}`} x={geo.cx} y={geo.cy+3} textAnchor="middle"
            style={{ fontSize:7, fontFamily:"Inter, system-ui", fontWeight:600, fill:highlightId===geo.id?"#fff":"#374151", pointerEvents:"none" }}>{geo.id}</text>
        ))}
      </svg>
      {tip && d && (
        <div style={{ position:"absolute", left:tip.x+12, top:tip.y-8, background:"#1A2E24", borderRadius:8, padding:"10px 14px", minWidth:160, boxShadow:"0 4px 16px rgba(0,0,0,0.18)", pointerEvents:"none", zIndex:20 }}>
      ...
      <div style={{ position:"absolute", bottom:8, right:8, background:"rgba(255,255,255,0.92)", borderRadius:6, padding:"7px 10px", border:"1px solid #E8E4DC" }}>
        <p style={{ fontSize:9, fontWeight:600, color:"#9CA3AF", letterSpacing:"0.06em", textTransform:"uppercase", marginBottom:4 }}>Predicted Demand</p>
        {[{c:"#F9C4A0",l:"High"},{c:"#FEF3C7",l:"Medium"},{c:"#D4ECD8",l:"Low"}].map(({c,l}) => (
          <div key={l} style={{ display:"flex", alignItems:"center", gap:5, marginBottom:2 }}>
            <div style={{ width:9, height:9, background:c, borderRadius:2, border:"1px solid rgba(0,0,0,0.1)" }}/><span style={{ fontSize:9, color:"#6B7280" }}>{l}</span>
          </div>
        ))}
      </div>
    </div>
...
const SAVED_INSIGHTS_INIT: SavedInsight[] = [
  {
    id:"I001",
...
function WorkspaceTab({ savedInsights, onSaveInsight, onOpenAnalysis }: {
...
export default function App() {
  ...
  function saveInsight(ins: SavedInsight) {
  ...
  function openAnalysis(anomalyId: string) {
    setNavigateAnomaly(anomalyId)
    setActiveTab("anomalies")
  }

  function handleTabChange(t: TabId) {
    if (t !== "anomalies") setNavigateAnomaly(null)
    setActiveTab(t)
  }
  ...
  return (
    <div style={{ background:"#F5F3EE", minHeight:"100vh", fontFamily:"Inter, system-ui, sans-serif" }}>
      ...
      {activeTab === "anomalies" && <AnomaliesTab onSaveInsight={saveInsight} initialSelected={navigateAnomaly}/>}
      {activeTab === "health"    && <HealthAMUTab/>}
      {activeTab === "forecast"  && <ForecastTab/>}
      {activeTab === "workspace" && <WorkspaceTab savedInsights={savedInsights} onSaveInsight={saveInsight} onOpenAnalysis={openAnalysis}/>}
    </div>
  )
}
